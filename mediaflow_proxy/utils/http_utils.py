...
async def get_shared_httpx_client() -> httpx.AsyncClient:
    ...
    async with _shared_client_lock:
        if _shared_client is None or _shared_client.is_closed:
            ...
            global _shared_client_creations
            _shared_client_creations += 1
            logger.info(f"Created new shared httpx.AsyncClient (count={_shared_client_creations})")
    return _shared_client
...
async def fetch_hls_segment_fast(url: str, headers: dict) -> bytes:
    ...
    # Inject a permissive Accept header if caller didn't set one (some disguised segments need it)
    if 'accept' not in {k.lower(): v for k, v in headers.items()}:
        headers = {**headers, 'Accept': 'video/*,application/octet-stream;q=0.9,*/*;q=0.8'}
    try:
